...
1. Select the `API permissions` section, and then click on `Add a permission`.  Note that you need a `Global Administrator` or `Privileged Role Administrator` Entra role assignment to configure App-level API permissions for Microsoft Graph.
